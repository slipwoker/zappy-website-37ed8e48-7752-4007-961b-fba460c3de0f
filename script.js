document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    const ERROR_MSG_CLASS = 'field-error-message';

    function getFieldWrapper(field) {
      return field.closest('.form-group, .form-field, [data-field]') || field.parentElement;
    }

    function clearFieldState(field) {
      const wrapper = getFieldWrapper(field);
      wrapper.classList.remove(ERROR_CLASS, SUCCESS_CLASS);
      const existingMsg = wrapper.querySelector('.' + ERROR_MSG_CLASS);
    ...
    function setFieldError(field, message) {
      ...
      wrapper.classList.remove(SUCCESS_CLASS);
    ...
    function setFieldSuccess(field) {
      ...
      wrapper.classList.add(SUCCESS_CLASS);
    ...
    function validateField(field) {
      ...
      if (isRequired && !validators.required(value)) {
        ...
        setFieldError(field, customMessage || fieldName + ' is required.');
        return false;
      }
      ...
      if (type === 'email' && !validators.email(value)) {
        setFieldError(field, customMessage || 'Please enter a valid email address.');
        return false;
      }

      if (type === 'tel' && !validators.phone(value)) {
        setFieldError(field, customMessage || 'Please enter a valid phone number.');
        return false;
      }

      if (minLength && !validators.minLength(value, minLength)) {
        setFieldError(field, customMessage || 'Please enter at least ' + minLength + ' characters.');
        return false;
      }
    ...
    function getValidatableFields() {
      return contactForm.querySelectorAll('input:not([type="hidden"]):not([type="submit"]):not([type="reset"]):not([type="button"]), textarea, select');
    }
    ...
    contactForm.addEventListener('input', function (e) {
      ...
      if (field.matches('input, textarea, select')) {
        const wrapper = getFieldWrapper(field);
        if (wrapper.classList.contains(ERROR_CLASS)) {
          validateField(field);
        }
      }
    });
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      fetch(action, {
        ...
        .then(function () {
          const successMsg = contactForm.getAttribute('data-success-message') || 'Thank you! Your message has been sent.';
          ...
          contactForm.insertAdjacentElement('afterend', successEl);
          contactForm.reset();
          getValidatableFields().forEach(function (field) { clearFieldState(field); });
        })
        .catch(function (error) {
          console.error('Form submission error:', error);
          const errorMsg = contactForm.getAttribute('data-error-message') || 'Something went wrong. Please try again.';
          const errorEl = document.createElement('div');
          errorEl.className = 'form-submission-error';
          errorEl.setAttribute('role', 'alert');
          errorEl.textContent = errorMsg;
          const existingError = contactForm.querySelector('.form-submission-error');
          if (existingError) existingError.remove();
          contactForm.insertAdjacentElement('beforebegin', errorEl);
        })
        .finally(function () {
          if (submitBtn) {
  ...
  const ANIMATION_CLASS = 'animate--visible';
  const OBSERVED_SELECTOR = '.animate-on-scroll, [data-animate]';
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "מצפן משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '37ed8e48-7752-4007-961b-fba460c3de0f',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '37ed8e48-7752-4007-961b-fba460c3de0f';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '37ed8e48-7752-4007-961b-fba460c3de0f';
